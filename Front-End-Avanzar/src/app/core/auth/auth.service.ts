...
export class AuthService {
  ...
  signIn(credentials: { username: string; password: string }): Observable<any> {
    ...
    return this._httpClient.post('https://164.90.153.70:8080/api/login/generartoken', credentials).pipe(
  ...
  signInUsingToken(): Observable<any> {
    ...
    return this._httpClient.post('https://164.90.153.70:8080/api/login/signInWithToken', {
  ...
  public getUsuarioActual(): Observable<any> {
    ...
    if (token) {
      ...
      return this._httpClient.get<any>('https://164.90.153.70:8080/api/login/usuarioActual', { headers: headers }).pipe(
        catchError((error) => {
          if (error.status === 401) {
